...
function Library:Notify(cfg)
    cfg = {
        ...
        Prefix = cfg.Prefix or "[Francium Rewrite] ",
...
function Library:TextTriggers(text)
    ...
    local Triggers = {
        ...
        ['{name}'] = ('<font color="rgb(%s, %s, %s)">Francium Rewrite</font>'):format(mathfloor(AccentTheme.r * 255), mathfloor(AccentTheme.g * 255), mathfloor(AccentTheme.b * 255)),
        ['{hour}'] = os.date("%H"),
        ['{minute}'] = os.date("%M"),
        ['{second}'] = os.date("%S"),
        ['{ap}'] = os.date("%p"),
...
function Library:Watermark(cfg)
    cfg = {
        Text = cfg.Text or "{name} | {ping}ms | {time}",
